...
(function () {
  ...
  const CONTACT = isMobile ? { x: 51, y: 50 } : { x: 52, y: 47 };
  ...
  if (microcopy) {
  window.setTimeout(() => {
    microcopy.classList.remove('is-highlight');
    void microcopy.offsetWidth;
    microcopy.classList.add('is-highlight');
...
/* Sincronia com a coreografia CSS:
   A coreografia do hi-five começa após 4.0s.
   O impacto visual acontece por volta de 75% do loop de 9s:
   4.0s + (9s * 0.75) = 10.75s.
*/
const FIRST_AT = 5160;
const REPEAT_EVERY = 7200;
